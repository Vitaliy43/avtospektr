...
function operation_manually(){
	
}

function operation(object,sign,url){


 buffer_array=object.split('_');
Id=buffer_array[1];

var container_quantity="quantity_"+Id;
var container_price="price_"+Id;
var container_sum="sum_"+Id;
var container_show_sum="tdsum_"+Id;
var container_quantity_store="quantitystore_"+Id;

var quantity=document.getElementById(container_quantity).value*1;
var price=document.getElementById(container_price).value*1;
var quantity_store=document.getElementById(container_quantity_store).value*1;
...
if(quantity>quantity_store){
	myAlert("Кол-во единиц в заказе превышает кол-во единиц на складе!");
	return false;
}
...
var change=change_quantity(url,Id,quantity,sign);
if(change==true){
	document.getElementById(container_quantity).value=quantity;
	...
	function change_quantity(url,id,quantity,sign){
		
	var container_quantity=$('#quantity_'+id).html();
    ...
    url: url,
	dataType: 'json',
	data: 'type=ajax&quantity='+quantity+'&order_id='+id+'&sign='+sign,
    cache: false,
	beforeSend:function(){
		$('#quantity_'+id).val('...');
	},
	error:function(){
		...
		$('#quantity_'+id).html(container_quantity);
	...
	if(data.answer==1){
		...
		$('#tdsum_'+id).html(data.sum);
		$('#sum_'+id).val(data.sum);
		return true;
	}
	else{
		myAlert('Неизвестная ошибка!');
    ...
    function change_sum(object,url){
...
var change=change_quantity(url,Id,quantity,'none');
if(change==true){
	document.getElementById(container_quantity).value=quantity;
	...
	return false;
         ...
         function get_totally(){
          ...
          for(var i=0;i<elements.length;i++){
            ...
            buffer_array=elements[i].id.split('_');
           ...
           if(Type=='check'){
                 ...
                 container_check=Type+'_'+Id;

                 if(document.getElementById(container_check).checked==true){

		  var container_sum="sum_"+Id;

		  sum+=document.getElementById(container_sum).value*1;
		...
		sum=roundPlus(sum,3);
          document.getElementById('totally').innerHTML='<div>'+sum+' руб.</div>';
		...
		function delete_from_basket(object,id){
			
			url=object.href;
			container=$('del_'+id+' a').html();
			jConfirm('Вы уверены, что хотите удалить данную запчасть?','Магазин "Автоспектр"',function(r){
    		...
    		cache: false,
			beforeSend:function(){
				$('#del_'+id+' a').html('<img src="/images/avtospektr/ajax-loaders/menu.gif">');

			},
    			success: function(data){
				
					$('#del_'+id+' a').html(container);
